...
CREATE TABLE IF NOT EXISTS `aesccar_gti09`.`huertos` (
  ...
  INDEX `UsuarioPropietario` (`UsuarioPropietario` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `aesccar_gti09`.`sondas` (
  ...
  INDEX `NumHuerto` (`NumHuerto` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `aesccar_gti09`.`sensores` (
  ...
  UNIQUE INDEX `TipoSensor` (`TipoSensor` ASC) VISIBLE,
  INDEX `NumSonda` (`NumSonda` ASC) VISIBLE,
  INDEX `Unidades` (`Unidades` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `aesccar_gti09`.`incidencias` (
  ...
  `NumSonda` INT(11) NOT NULL,
  PRIMARY KEY (`IdIncidencias`, `NivelGravedad`, `NumSensor`, `NumSonda`),
  INDEX `NumSensor` (`NumSensor` ASC) VISIBLE,
  INDEX `NumSonda` (`NumSonda` ASC) VISIBLE,
  CONSTRAINT `incidencias_ibfk_1`
    FOREIGN KEY (`NumSensor`)
    REFERENCES `aesccar_gti09`.`sensores` (`IdSensor`),
  CONSTRAINT `incidencias_ibfk_2`
    FOREIGN KEY (`NumSonda`)
    REFERENCES `aesccar_gti09`.`sensores` (`NumSonda`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4;
...
CREATE TABLE IF NOT EXISTS `aesccar_gti09`.`instalaciones` (
  ...
  INDEX `IdHuerto` (`IdHuerto` ASC) VISIBLE,
  INDEX `fk_instalaciones_sondas1_idx` (`IdSonda` ASC, `NumHuerto` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `aesccar_gti09`.`TecnicoEncargado` (
  ...
  PRIMARY KEY (`Tecnico`, `IdIncidencias`, `NivelGravedad`, `NumSensor`, `NumSonda`),
  INDEX `fk_incidencias_has_usuarios_usuarios1_idx` (`Tecnico` ASC) VISIBLE,
  INDEX `fk_incidencias_has_usuarios_incidencias1_idx` (`IdIncidencias` ASC, `NivelGravedad` ASC, `NumSensor` ASC, `NumSonda` ASC) VISIBLE,
  CONSTRAINT `fk_incidencias_has_usuarios_incidencias1`
    FOREIGN KEY (`IdIncidencias` , `NivelGravedad` , `NumSensor` , `NumSonda`)
    REFERENCES `aesccar_gti09`.`incidencias` (`IdIncidencias` , `NivelGravedad` , `NumSensor` , `NumSonda`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `aesccar_gti09`.`Mediciones` (
  ...
  INDEX `fk_Mediciones_sensores1_idx` (`IdSensor` ASC, `NumSonda` ASC, `Unidades` ASC) VISIBLE,
...
-- -----------------------------------------------------
-- Table `aesccar_gti09`.`Graficas`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `aesccar_gti09`.`Graficas` (
  `idGraficas` INT NOT NULL,
  `idMediciones` INT NOT NULL,
  `IdSensor` INT(11) NOT NULL,
  `NumSonda` INT(11) NOT NULL,
  `Unidades` INT(11) NOT NULL,
  PRIMARY KEY (`idGraficas`, `idMediciones`, `IdSensor`, `NumSonda`, `Unidades`),
  INDEX `fk_Graficas_Mediciones1_idx` (`idMediciones` ASC, `IdSensor` ASC, `NumSonda` ASC, `Unidades` ASC) VISIBLE,
  CONSTRAINT `fk_Graficas_Mediciones1`
    FOREIGN KEY (`idMediciones` , `IdSensor` , `NumSonda` , `Unidades`)
    REFERENCES `aesccar_gti09`.`Mediciones` (`idMediciones` , `IdSensor` , `NumSonda` , `Unidades`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
